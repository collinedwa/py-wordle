...
def game():
    ...
    while tries != 0:
        ...
        if guess.lower() == 'exit':
            ...
            # Iterates through each letter in the user submitted word and determine whether it matches the answer
            ...
            for i in range(5):
                temp_letter = word_dummy[i]
                if temp_letter in word_index and temp_letter == word_index[i]:
                    if temp_letter in list(available_letters.values()):
                        available_letters[word_dummy[i]] = (f"({word_dummy[i]})")

                    elif (f"[{word_dummy[i]}]") in list(available_letters.values()):
                        available_letters[temp_letter] = (f"({temp_letter})")

                    word_dummy[i] = (f"({temp_letter})")

                    # removes previous instances of [] if the guess letter is 
                    # in the correct spot at a later index

                    if ((word_dummy.count(f"[{temp_letter}]") + word_dummy.count(f"({temp_letter})")) 
                        > word_index.count(temp_letter)):

                        word_dummy[word_dummy.index(f"[{temp_letter}]")] = temp_letter

                elif temp_letter in word_index and temp_letter != word_index[i]:
                    if temp_letter in list(available_letters.values()):

                        available_letters[temp_letter] = (f"[{temp_letter}]")

                    if (word_dummy.count(f"[{temp_letter}]") < word_index.count(temp_letter) 
                        and (word_dummy.count(f"[{temp_letter}]") + word_dummy.count(f"({temp_letter})")) 
                        < word_index.count(temp_letter)):
